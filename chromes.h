...
void addChrome(int index,int kcal[4],int gram[4]){
	...
	cz[index].cArray[0][i] = kcal[i];
	cz[index].cArray[1][i] = gram[i];
}
	for(i=0;i<=1;i++){

		cz[index].total_energy = calculate_energy(cz[index].cArray);
		cz[index].total_quantity = calculate_quantity(cz[index].cArray);
	}
...
void print_chrome(chrome printCZ[], int size){
	...
	for(j = 0;j<2;j++){
		...
		for(k = 0;k < 4;k++){
			...
			printf("%d\t",printCZ[index].cArray[j][k]);
			
		}
		printf("\n");
	}
	printf("\nTotal Energy=\t%d Kcal\nTotal Weight=\t%d gram\n\n-----------------------------------------------\n\n",
	printCZ[index].total_energy,printCZ[index].total_quantity);
...
chrome copyc(chrome a){
    ...
    int i=0;
    for(i=0;i<4;i++){
        b.cArray[0][i]=a.cArray[0][i];

        b.cArray[1][i]=a.cArray[1][i];
